...
def test_that_isKingAttacked_returns_true_when_the_king_is_attacked():
    ...
    game.move('A7 to A5')
    game.move('E5 to E6')
    assert game.isKingAttacked(BLACK) == False
    game.move('A5 to A4')
    game.move('E6 to D7')
    assert game.isKingAttacked(BLACK) == True
...
def test_that_isKingAttacked_returns_false_when_the_king_is_not_attacked():
    ...
    game = Game(ruleSet, promotionCallback)
    game.load()
    assert game.isKingAttacked(WHITE) == False
...
def test_that_a_piece_is_untaken_during_undoLastMove():
    ...
    assert game.getPiece(Square(3, 4)).color == BLACK
